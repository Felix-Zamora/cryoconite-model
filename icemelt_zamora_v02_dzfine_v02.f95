...
! FZ 18Mar2018 - v07_dzfine
!			Decreased dz to 1 mm for top 30cm of ice columns, for a total of 540 cells.
!				(nz = 540).
!			Expanded icetempinit2008good.txt to include 350 values by creating new lines
!				between existing lines, using the value of the line above.
!			To account for sediment, the tops and bottom cells of the sediment layer
!				are defined (sdmnt_top, sdmnt_btm)
!			The sediment is moved when water_frac of the cell beneath sdmnt_btm equals 1.0.
! FZ 16Jul2018 - icemelt_zamora_v02_dzfine v01
!			Commented out TprcsXX (see comment from 05Feb2018) as they were consuming a lot of memory
!                and causing errors when the "fine" spacing was applied
!			Applied 0.001 m (1mm) grid spacing to upper 0.5 m of cell and adjusted cell size of grid beneath that accordingly,
!				 for a total of 540 cells.
! 			Updated mig_ablation threshold to 0.1 cm (1 mm).
!			Updated code so upper 500 mm were shifted up when mig_ablation threshold is reached.
! FZ 17Sep2018 - icemelt_zamora_v02_dzfine v02
!			Applied to cleaned icemelt_hoffman_v01.f95 baseprogram.
!			Restricted sediment to single 1 mm cell
! FZ 01Oct2018 - icemelt_zamora_v02_dzfine v02
!			Adjusted ablation calculation in SUBROUTINE MFENERGY so that ablation was calculated in mm, rather than cm
...
! 		Because the heat equation solver calls the z-dir(k) the y-dir(j).
    parameter(nz=540) !Options: 170, 37, 71, 70 FJZ + JMC found 70 to work
    parameter(JJ=nz)
	parameter(nx=200)
	parameter(ny=140)
	real deltaz(nz)
...
!		real Tprcs1(JJ+2),Tprcs2(JJ+2),Tprcs3(JJ+2)
    ...
    real down(JJ+2)
		...
		real sdmnt_out ! used to make sure sdmnt output is in proper format
		integer sdmnt
    integer :: maxiter
...
! Otherwise run default parameters below
        ...
        runnametext = "DEFAULT_NAME"
				...
				sdmnt = 250  ! initialize sediment depth
				mig_ablation = 0.0 ! initialize ablation affecting sediment depth
				net_ablation = 0.0 ! initialize net ablation
        drainthresh = 1.00  ! the water frac at which water is removed from subsurface
...
! cell thickness is 0.10 m at 1.0m depth.
	...
	elseif (nz.eq.540) then
		deltaz(2:500) = 0.001
		do j=501,540
			deltaz(j)=deltaz(j-1)*1.217093
		enddo
		deltaz(540)=deltaz(540)-deltaz(1)+0.01
...
!				'unformatted',recl=iwordlength*1*(JJ+3))
	open (87,file='./output/'//runname(1:strlen(runname)) &
				...
				, access='direct',form= &
				'unformatted',recl=iwordlength*1*(2))
...
!---------------------------------------------------------------------
	...
	ifinalcall = 1 !Iterating is complete
            CALL ICE_ENERGY(sdmnt,gamma,T_old,Tsfc,JJ,dy_p,y_crds, &
...
! write sdmnt depth and net ablation info
		...
		write(87,rec=iarraypos) &
			(sed_ablation(i,iarraypos),i=1,2)
...
!	Move cryoconite sediments down one cell if cell beneath it has fully melted.
!  Also, change water_frac of old sediment cell to 1.0.

		if (water_frac(sdmnt+1).ge.1.0) then
			sdmnt = sdmnt + 1
			water_frac(sdmnt-1) = 1.0
		endif

! Move cryoconite sediments up one cell if ablation exceeds 1.0 cm. Then reset mig_ablation
		mig_ablation = mig_ablation + ablation
...
!=====================================================================
      ...
      SUBROUTINE ICE_ENERGY(sdmnt,gamma,T_old,Tsfc,JJ,dy_p,y_crds, &
        dt,f_n,dely_p,Qsip,Qsi,albedo,extcoef,xk_snow, &
        ro_snow,Cp_snow,xk_water,water_frac,up,down,total_solar, &
        xLs,Rv,Tf,ro_water,xmelt,ro_ice,water_depth, &
        water_depth_old,water_flux,xLf,ro_pure_ice, &
...
! Solve the ice temperature equation.
      CALL ICEHEAT(sdmnt,gamma,T_old,Tsfc,JJ,dy_p,y_crds, &
        dt,f_n,dely_p,Qsip,Qsi,albedo,extcoef,xk_snow, &
        ro_snow,Cp_snow,xk_water,water_frac,up,down,total_solar, &
...
!   with the case of refreezing water.
      CALL ICEMF(sdmnt,T_old,JJ,dy_p,xmelt,Cp_snow,xLf,Tf,ro_ice, &
...
! Disable Step 3 for now.  (Eventually should delete and clean up.)
      if (flag.eq.9999.0) then  ! start step 3 if-construct
        CALL GETNEWT(sdmnt,gamma,T_old,Tsfc,JJ,dy_p,y_crds, &
          dt,f_n,dely_p,Qsip,Qsi,albedo,extcoef,xk_snow, &
...
!=====================================================================
      ...
      SUBROUTINE ICEMF(sdmnt,T_old,JJ,dy_p,xmelt,Cp_snow,xLf,Tf,ro_ice, &
      ro_snow,water_frac,flag,ro_pure_ice)
...
!=====================================================================
!=====================================================================
      SUBROUTINE GETNEWT(sdmnt,gamma,T_old,Tsfc,JJ,dy_p,y_crds, &
        dt,f_n,dely_p,Qsip,Qsi,albedo,extcoef,xk_snow, &
        ro_snow,Cp_snow,T_tmp,Tf,xk_water,water_frac, &
        up,down,total_solar,xLs,Rv,ro_water,ro_pure_ice, &
        xinternal_heating_corr,ndarklayers,Sc)
...
! Compute gamma and the general equation coefficients.
      CALL GETGAMMA(sdmnt,gamma,JJ,xk_snow,xk_water,water_frac, &
        T_old,xLs,Rv,Tf,ro_snow,ro_water,ro_pure_ice,albedo)
      CALL GAMMA1(g_b_ns,gamma,f_n,JJ)
      CALL GE_COEF(sdmnt,aN,aS,aP0,dy_p,dely_p,g_b_ns,dt,JJ, &
        ro_snow,Cp_snow)
...
!=====================================================================
      ...
      SUBROUTINE ICEHEAT(sdmnt,gamma,T_old,Tsfc,JJ,dy_p,y_crds, &
        dt,f_n,dely_p,Qsip,Qsi,albedo,extcoef,xk_snow, &
        ro_snow,Cp_snow,xk_water,water_frac,up,down,total_solar, &
        xLs,Rv,Tf,ro_water,ro_pure_ice,xinternal_heating_corr, &
        ndarklayers,Sc)
...
!=====================================================================
      ...
      SUBROUTINE GETGAMMA(sdmnt,gamma,JJ,xk_snow,xk_water,water_frac, &
        T_old,xLs,Rv,Tf,ro_snow,ro_water,ro_pure_ice,albedo)
...
!=====================================================================
      ...
      SUBROUTINE GE_COEF(sdmnt,aN,aS,aP0,dy_p,dely_p,g_b_ns,dt,JJ, &
        ro_snow,Cp_snow)
...
! Be sure that ICE_ENERGY and CONDUCT aren't using a Tsfc>0
	...
	qsfactor = -99999.0 ! This can be a dummy value - make it crash if used
      ...
      CALL ICE_ENERGY(sdmnt,gamma_temp,T_old_temp,T_sfcguess,JJ,dy_p,y_crds, &
              dt,f_n,dely_p,Qsip,Qsi,albedo,extcoef,xk_snow, &
